...
- [x] Direct PTY launch remains the default and works without tmux installed or
      configured.
- [x] Keep-alive is selectable only from a ready server capability and sends
      only a boolean alongside the existing fixed launch input.
- [x] The server generates the tmux name and launches only one available
      fixed preset through direct multiple `new-session` arguments without a
      shell or browser command authority.
- [x] Launch has bounded timeout/output, strict target parsing, canonical cwd,
      safe socket location, collision failure, and no automatic retry after an
      unknown browser outcome.
- [x] A keep-alive launch creates one tmux session, one Pacium client PTY, one
...
- [x] Shell, Codex, and Claude retain honest preset classification; native
      provider observation is explicitly unavailable for the tmux runtime
      rather than silently inferred or falsely reconnected.
- [x] Browser refresh/view close preserves the client; deliberate client close
      and local-server shutdown leave the tmux target alive.
- [x] Local-server restart automatically reattaches each newest unique
      keep-alive target once with a fresh Pacium session ID and predecessor
      lineage.
- [x] Direct manifests, external attachment manifests, missing targets, and
      duplicate/stale lineage are never automatically launched or retried.
- [x] A target that exits or disappears remains a retained failed recovery
      option; Pacium does not rerun its underlying command automatically.
- [x] Focused, integration, real-tmux, restart, security, full verification,
...
- Server shutdown awaits an exact client identity detach and never invokes
  `kill-session`.
...
Completed on 2026-07-28. The real tmux/PTY lifecycle canary covers fixed-preset
launch, terminal input, exact client detach, manager restart, fresh linked
identity, deliberate close, and surviving target state. Focused contract,
manifest, adapter, manager, transport, WebSocket, UI, failure, exclusion, and
security tests pass. `pnpm verify` passed 131 files and 859 tests with 949.29 kB
web JavaScript, 122.44 kB CSS, and 457.78 kB local-server JavaScript;
`pnpm test:e2e` passed all 19 Chromium workflows. Verification used Node.js
26.4.0 rather than supported Node.js 24.18.x, and no PC-072 through PC-076
