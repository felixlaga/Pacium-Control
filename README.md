...
no-follow stable reads and canonical-parent watches; original UTF-8 text stays
in local-server memory while the browser sees only source health plus
whole-source type, confidence, deterministic identity, and fixed diagnostic
metadata. Pacium never executes or writes queue contents, and classification
grants no approval authority. Browser prompts do not use those paths; they send
bounded input directly to one explicitly selected live PTY. See the
