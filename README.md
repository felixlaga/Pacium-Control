...
in local-server memory while the browser sees only source-health metadata.
Pacium never executes or writes queue contents. Browser prompts do not use
those paths; they send bounded input directly to one explicitly selected live
PTY. See the
