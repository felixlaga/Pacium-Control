...
keeps at most one decoded item and renders it only as inert text. Pacium never
executes or writes queue contents, and classification or inspection grants no
approval authority. Browser prompts do not use those paths; they send bounded
input directly to one explicitly selected live PTY. See the
